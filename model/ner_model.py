...
class NERModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.char_lstm = DPLSTM(self.config.dim_char, self.config.hidden_size_char, bidirectional=True)
        self.char_lstm = nn.LSTM(self.config.dim_char, self.config.hidden_size_char, bidirectional=True)

        self.dropout = nn.Dropout(p=self.config.dropout)
        # self.word_lstm = DPLSTM(self.config.dim_word + 2*self.config.hidden_size_char + self.config.dim_elmo,
        ...
        #                          self.config.hidden_size_lstm, bidirectional=True)
        self.word_lstm = nn.LSTM(self.config.dim_word + 2*self.config.hidden_size_char + self.config.dim_elmo,
                                 self.config.hidden_size_lstm, bidirectional=True)

        self.linear = LinearClassifier(self.config, layers=[self.config.hidden_size_lstm*2, self.config.ntags], drops=[0.5])


    def forward(self, input, extracting=False):
        ...
        # else:
        word_input, char_input, elmo_emb = input[0], input[1], input[2]
        word_input.transpose_(0,1)
        elmo_emb.transpose_(0,1)

        # DO THIS FOR OTHER EMBEDDINGS 
        # Word Embedding
        ...
        # Concat char output and word output AND NOW ELMO OUTPUT
        # word_emb = torch.cat((word_emb, char_output), 2) #shape = S*B*(wnh+2cnh)
        word_emb = torch.cat((word_emb, char_output, elmo_emb), 2) #shape = S*B*(wnh+2cnh + stuff)
        
        
        #catch intermediate model output
        if extracting ==True:
            embs = word_emb
            embs = embs.cpu()
            embs = embs.detach().numpy()
            #transpose to BSx seq len x embs dim
            embs = np.transpose(embs, (1, 0, 2))
        ...
        return output, None #shape = S*B*ntags
